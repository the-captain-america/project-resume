...
class AutoComplete extends React.Component {
  constructor (props) {
    ...
    this.state = {
      ...
      options: this.props.options, // options can be updated
      value: ''
    }
  }
   ...
   _supportsNative () {
    ...
    return Boolean(feature && feature.options);
     //returns a boolean eg: true/false
     // dependent upon feature exists && feature.options
  }
  ...
  _renderFallbackOptions () {
    let options = this.state.shownOptions.map((option, index) => {
      ...
      return (
        <li className={classNames}
          ...
          role='option'>
          {option.option}
          <i id="filter_submit" className="fa fa-search"></i>
        </li>
      )
    })
    ...
    return (
      <ul style={this.props.style} aria-multiselectable='false' className={classNames} role='listbox' style={{width: this.state.inputWidth}}>
        {options}
      </ul>
    )
  }
